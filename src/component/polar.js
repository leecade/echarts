...
Polar.prototype = {
    ...
    _getRadius: function () {
        ...
        item.radius, Math.min(this.zr.getWidth(), this.zr.getHeight()) / 2);
    },
    ...
    _buildSpiderWeb: function (index) {
        ...
        this._addArea(
        __ecIndicator, splitNumber, center, splitArea, strokeColor, lineWidth, show);
    ...
    _buildText: function (index) {
        ...
        for (var i = 0; i < indicator.length; i++) {
            name = this.deepQuery([indicator[i], item, this.option], 'name');

            if (!name.show) {
                continue;
            }
            textStyle = this.deepQuery([name, item, this.option], 'textStyle');
            ...
            if (name.rotate) {
                rotation = [
                name.rotate / 180 * Math.PI, vector[0], vector[1]];
            }
            else {
                rotation = [0, 0, 0];
            }
    ...
    _mapVector: function (vector, center, scale) {
        return [
        vector[0] * scale + center[0], vector[1] * scale + center[1]];
    },
    ...
    __ecIndicator, center, axisLine) {
        ...
        for (var i = 0; i < indLen; i++) {
            vector = __ecIndicator[i].vector;
            line = this._getLine(
            center[0], center[1], vector[0] + center[0], vector[1] + center[1], strokeColor, lineWidth, lineType);
    ...
    _getSeriesData: function (index) {
        ...
        for (var i = 0; i < this.series.length; i++) {
            ...
            for (var j = 0; j < serieData.length; j++) {
                polarIndex = this.deepQuery([serieData[j], serie, this.option], 'polarIndex') || 0;
