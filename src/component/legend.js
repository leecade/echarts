...
Legend.prototype = {
    ...
    _buildItem: function () {
        ...
        for (var i = 0; i < dataLength; i++) {
            ...
            lastX, lastY, itemWidth, itemHeight, (this._selectedMap[itemName] && this._hasDataMap[itemName] ? color : '#ccc'), itemType, color);
    ...
    _mLineOptimize: function () {
        ...
        for (var i = 2, l = this.shapeList.length; i < l; i++) {
            if (this.shapeList[i].style.x === lastX) {
                ...
                this.shapeList[i - 1].style.text, this.shapeList[i - 1].style.textFont) - lastX)) / 2);
            }
            else if (i === l - 1) {
                lineOffsetArray.push((
                this._itemGroupLocation.width - (
                this.shapeList[i].style.x + zrArea.getTextWidth(
                this.shapeList[i].style.text, this.shapeList[i].style.textFont) - lastX)) / 2);
            }
        }
        var curLineIndex = -1;
    ...
    _getItemGroupLocation: function () {
        ...
        if (this.legendOption.orient === 'horizontal') {
            ...
            for (var i = 0; i < dataLength; i++) {
                ...
                this._getFormatterNameFromData(data[i]), data[i].textStyle ? this.getFont(zrUtil.merge(
                data[i].textStyle || {}, textStyle)) : font);
    ...
    _getItemShapeByType: function (x, y, width, height, color, itemType, defaultColor) {
        ...
        case 'k':
            ...
            itemShape.style.color = this.deepQuery([this.ecTheme, ecConfig], 'k.itemStyle.normal.color') || '#fff';
            itemShape.style.strokeColor = color != '#ccc' ? (
            this.deepQuery([this.ecTheme, ecConfig], 'k.itemStyle.normal.lineStyle.color') || '#ff3200') : color;
            break;
        case 'image':
    ...
    __legendSelected: function (param) {
        ...
        ecConfig.EVENT.LEGEND_SELECTED, param.event, {
    ...
    __dispatchHoverLink: function (param) {
        this.messageCenter.dispatch(
        ecConfig.EVENT.LEGEND_HOVERLINK, param.event, {
        ...
        if (newOption) {
            ...
            for (var i = 0, dataLength = data.length; i < dataLength; i++) {
                ...
                if (!something.series) {
                    ...
                    this.deepQuery(queryTarget, 'itemStyle.normal.color'), something.seriesIndex, something.dataIndex, something.data);
    ...
    setSelected: function (itemName, selectStatus) {
        ...
        this.messageCenter.dispatch(
        ecConfig.EVENT.LEGEND_SELECTED, null, {
            selected: this._selectedMap,
            target: itemName
        }, this.myChart);
    },
