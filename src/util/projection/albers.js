...
function _albers() {
    ...
    function albers(coordinates) {
        ...
        var p = Math.sqrt(
        C - 2 * n * Math.sin(radians * coordinates[1])) / n;
        return [
        scale * p * Math.sin(t) + translate[0], scale * (p * Math.cos(t) - p0) + translate[1]];
    }
    ...
    albers.invert = function (coordinates) {
        ...
        return [(lng0 + t / n) / radians, Math.asin((C - p * p * n * n) / (2 * n)) / radians];
    };
