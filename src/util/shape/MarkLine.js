...
MarkLine.prototype = {
    ...
    buildLinePath: function (ctx, style) {
        ...
        var len = Math.min(
        style.pointList.length, Math.round(style.pointListLength || style.pointList.length));
        ...
        if (!style.lineType || style.lineType == 'solid') {
            ...
            if (style.smooth !== 'spline') {
                ...
                for (var i = 1; i < len; i++) {
                    dashedLineTo(
                    ctx, pointList[i - 1][0], pointList[i - 1][1], pointList[i][0], pointList[i][1], dashLength);
                }
            }
            else {
                // 曲线
    ...
    brushSymbol: function (ctx, style, idx) {
        ...
        if (rotate !== 0) {
            ...
            matrix.rotate(
            transform, transform, rotate * Math.PI / 180);
            if (x || y) {
                matrix.translate(transform, transform, [x, y]);
            }
    ...
    buildArrawPath: function (ctx, style, idx) {
        ...
        var point = [
            [
            x + symbolSize * Math.cos(rotate - halfRotate), y - symbolSize * Math.sin(rotate - halfRotate)],
            [
            x + symbolSize * 0.6 * Math.cos(rotate), y - symbolSize * 0.6 * Math.sin(rotate)],
            [
            x + symbolSize * Math.cos(rotate + halfRotate), y - symbolSize * Math.sin(rotate + halfRotate)]
        ];
        ctx.moveTo(x, y);
