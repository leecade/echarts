...
var canvasSupported = require('../zrender/tool/env.js').canvasSupported;
...
function point(zr, effectList, shape, zlevel) {
    ...
    if (canvasSupported) { // 提高性能，换成image
        ...
        effectShape, effectShape.style.width + shadowBlur * 2 + 2, effectShape.style.height + shadowBlur * 2 + 2).style.image;
    ...
    zr.animate(effectShape.id, '', effect.loop).when(
    duration, {
...
function largePoint(zr, effectList, shape, zlevel) {
    ...
    for (var i = 0; i < 20; i++) {
        ...
        zr.animate(effectShape.id, 'style', true).when(duration, clip1).when(duration * 2, clip2).when(duration * 3, clip1).when(duration * 4, clip1).delay(Math.random() * duration * i)
...
function line(zr, effectList, shape, zlevel) {
    ...
    if (canvasSupported) { // 提高性能，换成image
        effectShape.style.image = zr.shapeToImage(
        effectShape, (size + shadowBlur) * 2, (size + shadowBlur) * 2).style.image;
        effectShape = new ImageShape({
            zlevel: effectShape.zlevel,
            style: effectShape.style,
            draggable: false,
    ...
    if (!shape.style.smooth) {
        // 直线
        zr.animate(effectShape.id, 'style', effect.loop).when(
        duration, {
            x: shape._x - offset,
            y: shape._y - offset
        }).done(function () {
