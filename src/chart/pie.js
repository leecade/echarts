...
function Pie(ecTheme, messageCenter, zr, option, myChart) {
    ...
    self.shapeHandler.onmouseover = function (param) {
        ...
        seriesIndex, dataIndex, percent, lastAddRadius, startAngle, endAngle, defaultColor, true);
        if (label) {
        ...
        seriesIndex, dataIndex, lastAddRadius, shape.style.r0, shape.style.r, startAngle, endAngle, defaultColor, true);
        if (labelLine) {
...
Pie.prototype = {
    ...
    _buildSinglePie: function (seriesIndex) {
        ...
        for (var i = 0, l = data.length; i < l; i++) {
            ...
            if (i > 0 && Math.abs(startAngle - endAngle) < 15 // 约15度
            && lastPercent < 4 && this._needLabel(serie, data[i], false) && this.deepQuery([data[i], serie], 'itemStyle.normal.label.position') != 'center') {
                // 都小就延长，前小后大就缩短
            ...
            data[i].selected, r0, r1, startAngle, endAngle, defaultColor);
            if (!clockWise) {
                startAngle = endAngle;
            }
        }
     ...
     */
    _buildItem: function (
    seriesIndex, dataIndex, percent, lastAddRadius, isSelected, r0, r1, startAngle, endAngle, defaultColor) {
        var series = this.series;
        // 扇形
        var sector = this.getSector(
        seriesIndex, dataIndex, percent, isSelected, r0, r1, startAngle, endAngle, defaultColor);
        // 图形需要附加的私有数据
        ecData.pack(
        sector, series[seriesIndex], seriesIndex, series[seriesIndex].data[dataIndex], dataIndex, series[seriesIndex].data[dataIndex].name, percent);
        sector._lastAddRadius = lastAddRadius;
        this.shapeList.push(sector);

        // 文本标签，需要显示则会有返回
        var label = this.getLabel(
        seriesIndex, dataIndex, percent, lastAddRadius, startAngle, endAngle, defaultColor, false);
        if (label) {
            ecData.pack(
            label, series[seriesIndex], seriesIndex, series[seriesIndex].data[dataIndex], dataIndex, series[seriesIndex].data[dataIndex].name, percent);
        ...
        seriesIndex, dataIndex, lastAddRadius, r0, r1, startAngle, endAngle, defaultColor, false);
        if (labelLine) {
            ecData.pack(
            labelLine, series[seriesIndex], seriesIndex, series[seriesIndex].data[dataIndex], dataIndex, series[seriesIndex].data[dataIndex].name, percent);
     ...
     */
    getLabel: function (
    seriesIndex, dataIndex, percent, lastAddRadius, startAngle, endAngle, defaultColor, isEmphasis) {
        var series = this.series;
        var serie = series[seriesIndex];
        var data = serie.data[dataIndex];
        ...
        zrUtil.clone(data.itemStyle) || {}, serie.itemStyle);
        // label配置
    ...
    getLabelText: function (seriesIndex, dataIndex, percent, status) {
        ...
        if (formatter) {
            if (typeof formatter == 'function') {
                ...
                this.myChart, serie.name, data.name, data.value, percent);
            }
    ...
    seriesIndex, dataIndex, lastAddRadius, r0, r1, startAngle, endAngle, defaultColor, isEmphasis) {
        var series = this.series;
        var serie = series[seriesIndex];
        var data = serie.data[dataIndex];
        ...
        if (this._needLabelLine(serie, data, isEmphasis)) {
            ...
            var midRadius = r1;
            // 视觉引导线终点半径
            var maxRadius = this.parseRadius(this.zr, serie.radius)[1] - (-labelLineControl.length) + lastAddRadius;
            ...
            return new BrokenLineShape({
                ...
                style: {
                    pointList: [
                        [
                        centerX + midRadius * cosValue, centerY - midRadius * sinValue],
                        [
                        centerX + maxRadius * cosValue, centerY - maxRadius * sinValue],
                        [
                        data.__labelX, data.__labelY]
                    ],
    ...
    _needLabel: function (serie, data, isEmphasis) {
        return this.deepQuery([data, serie], 'itemStyle.' + (isEmphasis ? 'emphasis' : 'normal') + '.label.show');
    },
    ...
    _needLabelLine: function (serie, data, isEmphasis) {
        return this.deepQuery([data, serie], 'itemStyle.' + (isEmphasis ? 'emphasis' : 'normal') + '.labelLine.show');
    },
    ...
    reformOption: function (opt) {
        ...
        opt.itemStyle.emphasis.label.textStyle || {}, this.ecTheme.textStyle);
        ...
        for (var i = 0, l = params.length; i < l; i++) {
            ...
            if (series[seriesIndex] && series[seriesIndex].type == ecConfig.CHART_TYPE_PIE) {
                if (isHead) {
                    if (!dataGrow) {
                        sectorMap[
                        seriesIndex + '_' + series[seriesIndex].data.length] = 'delete';
                    }
            ...
            if (aniMap[seriesIndex]) {
                ...
                if (backupShapeList[i].type == 'sector') {
                    if (targeSector != 'delete') {
                        // 原有扇形
                        this.zr.animate(backupShapeList[i].id, 'style').when(
                        400, {
    ...
    onclick: function (param) {
        ...
        for (var i = 0, len = this.shapeList.length; i < len; i++) {
            if (this.shapeList[i].id == target.id) {
                ...
                if (!style._hasSelected) {
                    ...
                    offset = this.query(
                    series[seriesIndex], 'selectedOffset');
        ...
        ecConfig.EVENT.PIE_SELECTED, param.event, {
