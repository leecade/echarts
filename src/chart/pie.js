...
function Pie(ecTheme, messageCenter, zr, option, myChart) {
    ...
    self.shapeHandler.onmouseover = function (param) {
        ...
        seriesIndex, dataIndex, percent, center, midAngle, defaultColor, true);
        ...
        seriesIndex, dataIndex, center, shape.style.r0, shape.style.r, midAngle, defaultColor, true);
...
Pie.prototype = {
    ...
    _buildSinglePie: function (seriesIndex) {
        ...
        for (var i = 0, l = data.length; i < l; i++) {
            ...
            this._buildItem(
            singleShapeList, seriesIndex, i, percent, data[i].selected, center, r0, r1, startAngle, endAngle, defaultColor);
            if (!clockWise) {
                startAngle = endAngle;
            }
        }
     ...
     */
    _buildItem: function (
    singleShapeList, seriesIndex, dataIndex, percent, isSelected, center, r0, r1, startAngle, endAngle, defaultColor) {
        var series = this.series;
        var midAngle = ((endAngle + startAngle) / 2 + 360) % 360; // 中值

        // 扇形
        var sector = this.getSector(
        seriesIndex, dataIndex, percent, isSelected, center, r0, r1, startAngle, endAngle, defaultColor);
        // 图形需要附加的私有数据
        ecData.pack(
        ...
        seriesIndex, dataIndex, percent, center, midAngle, defaultColor, false);
        // 文本标签视觉引导线，需要显示则会有返回
        var labelLine = this.getLabelLine(
        seriesIndex, dataIndex, center, r0, r1, midAngle, defaultColor, false);
     ...
     */
    getLabel: function (
    seriesIndex, dataIndex, percent, center, midAngle, defaultColor, isEmphasis) {
        var series = this.series;
        var serie = series[seriesIndex];
        var data = serie.data[dataIndex];
    ...
    seriesIndex, dataIndex, center, r0, r1, midAngle, defaultColor, isEmphasis) {
        var series = this.series;
        var serie = series[seriesIndex];
        var data = serie.data[dataIndex];
        ...
        if (this._needLabelLine(serie, data, isEmphasis)) {
            ...
            return new PolylineShape({
                ...
                style: {
                    pointList: [
                        [
                        centerX + minRadius * cosValue, centerY - minRadius * sinValue],
                        [
    ...
    _layoutCalculate: function (tList, center, r, direction) {
        ...
        function _changeX(sList, isDownList, center, r, direction) {
            ...
            for (var i = 0, l = sList.length; i < l; i++) {
                ...
                deltaX = (deltaY < r + length) ? Math.sqrt((r + length + 20) * (r + length + 20) - Math.pow(sList[i]._rect.y - y, 2)) : Math.abs(
    ...
    reformOption: function (opt) {
        ...
        opt = _merge(
        _merge(
        opt || {}, zrUtil.clone(this.ecTheme.pie || {})), zrUtil.clone(ecConfig.pie));
