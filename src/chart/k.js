    ...
    _buildHorizontal: function (seriesArray, maxDataLength, locationMap) {
        ...
        for (var j = 0, k = locationMap.length; j < k; j++) {
            ...
            for (var i = 0, l = maxDataLength; i < l; i++) {
                ...
                candleWidth, valueAxis.getCoord(value[0]), // 纵坐标：开盘
...
/*
                ...
                for (var i = 0, l = seriesPL.length; i < l; i++) {
                    ...
                    queryTarget, 'itemStyle.normal.color0') || nColor0),

                    // 线宽
                    this.query(
                    queryTarget, 'itemStyle.normal.lineStyle.width') || nLineWidth,

                    // 线色
                    candleType ? (this.query( // 阳
                    queryTarget, 'itemStyle.normal.lineStyle.color') || nLineColor) : (this.query( // 阴
                    queryTarget, 'itemStyle.normal.lineStyle.color0') || nLineColor0),
                    ...
                    queryTarget, 'itemStyle.emphasis.color0') || eColor0 || nColor0),

                    // 线宽
                    this.query(
                    queryTarget, 'itemStyle.emphasis.lineStyle.width') || eLineWidth || nLineWidth,
     ...
     */
    _getCandle: function (
    seriesIndex, dataIndex, name, x, width, y0, y1, y2, y3, nColor, nLinewidth, nLineColor, eColor, eLinewidth, eLineColor) {
        var series = this.series;
        var itemShape = {
            zlevel: this.getZlevelBase(),
            z: this.getZBase(),
            clickable: this.deepQuery([series[seriesIndex].data[dataIndex], series[seriesIndex]], 'clickable'),
            hoverable: this.deepQuery([series[seriesIndex].data[dataIndex], series[seriesIndex]], 'hoverable'),
            style: {
                x: x,
                y: [y0, y1, y2, y3],
                width: width,
        ...
        ecData.pack(
        itemShape, series[seriesIndex], seriesIndex, series[seriesIndex].data[dataIndex], dataIndex, name);
