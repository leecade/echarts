...
Chord.prototype = {
    ...
    _buildShape: function () {
        ...
        this.chordSerieSample.radius[0], zrSize / 2);
        this.outerRadius = this.parsePercent(
        this.chordSerieSample.radius[1], zrSize / 2);
        ...
        this.center = [
        this.parsePercent(this.chordSerieSample.center[0], zrWidth), this.parsePercent(this.chordSerieSample.center[1], zrHeight)];
        var fixSize =
        this.chordSerieSample.itemStyle.normal.chordStyle.lineStyle.width - this.chordSerieSample.itemStyle.normal.lineStyle.width;
        this.strokeFix = (fixSize / _devicePixelRatio) / this.innerRadius / Math.PI * 180;
        ...
        case 'ascending':
        case 'descending':
            var groupIndices = groupAngles.argsort(0, this.sortGroups);
        ...
        case 'ascending':
        case 'descending':
            var subGroupIndices = subGroupAngles.argsort(1, this.sortSubGroups);
            subGroupAngles['sort'](1, this.sortSubGroups);
            break;
        default:
            var subGroupIndices = NDArray.range(subGroupNumber).reshape(1, subGroupNumber).repeat(groupNumber, 0);
        }
        ...
        groupNumber, subGroupNumber).toArray();
        var values = [];
        var start = 0;
        var end = 0;
        for (var i = 0; i < groupNumber; i++) {
            ...
            for (var j = 0; j < subGroupNumber; j++) {
                ...
                var subSortedIndex = subGroupIndicesArr[sortedIdx][j]; /*jshint maxlen : 200*/
                chordAngles[sortedIdx][subSortedIndex] = [subStart, subEnd];
                subStart = subEnd;
            }
        ...
        // reset data
        this.chordShapes = new NDArray(groupNumber, groupNumber, serieNumber).toArray();
        this.sectorShapes = [];

        this._buildSectors(sectorAngles, values);

        chordAngles = new NDArray(chordAngles).reshape(
        groupNumber, groupNumber, serieNumber, 2).toArray();
        this._buildChords(chordAngles, this.dataMat.reshape(shape).toArray());

        var res = this.normalizeValue(values);
        if (this.showScale) {
            this._buildScales(
            res[0], res[1], sectorAngles, new NDArray(res[0]).sum() / (360 - this.padding * groupNumber));
        }
    ...
    _buildSectors: function (angles, data) {
        ...
        function createMouseOver(idx) {
            return function () {
                ...
                timeout = setTimeout(function () {
                    for (var i = 0; i < len; i++) {
                        self.sectorShapes[i].style.opacity = i === idx ? 1 : 0.1;
                        ...
                        for (var j = 0; j < len; j++) {
                            for (var k = 0; k < len2; k++) {
                                ...
                                if (chordShape) {
                                    chordShape.style.opacity = (i === idx || j === idx) ? 0.5 : 0.03;
        ...
        for (var i = 0; i < len; i++) {
            ...
            sector.style.lineWidth = this.deepQuery([group, this.chordSerieSample], 'itemStyle.normal.lineStyle.width');
            sector.highlightStyle.lineWidth = this.deepQuery([group, this.chordSerieSample], 'itemStyle.emphasis.lineStyle.width');
            sector.style.strokeColor = this.deepQuery([group, this.chordSerieSample], 'itemStyle.normal.lineStyle.color');
            sector.highlightStyle.strokeColor = this.deepQuery([group, this.chordSerieSample], 'itemStyle.emphasis.lineStyle.color');
            ...
            ecData.pack(
            sector, this.chordSeries[0], 0, data[i], i, group.name);
            if (showLabel) {
                var halfAngle = [_start + _end] / 2;
                halfAngle %= 360; // Constrain to [0,360]
                var isRightSide = halfAngle <= 90 || halfAngle >= 270;
                ...
                labelShape.style.textColor = this.deepQuery([group, this.chordSerieSample], 'itemStyle.normal.label.textStyle.color') || '#fff';
                labelShape.style.textFont = this.getFont(this.deepQuery([group, this.chordSerieSample], 'itemStyle.normal.label.textStyle'));
    ...
    _buildChords: function (angles, dataArr) {
        ...
        var chordLineStyle = this.chordSerieSample.itemStyle.normal.chordStyle.lineStyle;
        var chordLineStyleEmphsis = this.chordSerieSample.itemStyle.emphasis.chordStyle.lineStyle;
        ...
        for (var i = 0; i < len; i++) {
            for (var j = 0; j < len; j++) {
                for (var k = 0; k < len2; k++) {
                    ...
                    if (angleIJ0 - angleJI1 === 0 || angleJI0 - angleJI1 === 0) {
                    ...
                    ecData.pack(
                    chord, this.chordSeries[k], k, dataArr[i][j][k], i + '-' + j, this.groups[i].name, this.groups[j].name, dataArr[j][i][k]);
    ...
    values, unitPostfix, angles, unitValue) {
        for (var i = 0; i < angles.length; i++) {
            ...
            while (scaleAngle < subEndAngle) {
                var thelta = ((this.clockWise ? (360 - scaleAngle) : scaleAngle) + this.startAngle) / 180 * Math.PI;
                var v = [
                Math.cos(thelta), -Math.sin(thelta)];
            ...
            while (scaleTextAngle < subEndAngle) {
                var thelta = this.clockWise ? (360 - scaleTextAngle) : scaleTextAngle;
                thelta = (thelta + this.startAngle) % 360;
                var isRightSide = thelta <= 90 || thelta >= 270;
                var textShape = {
                    zlevel: this._zlevelBase - 1,
                    hoverable: false,
                    style: {
                        x: isRightSide ? this.outerRadius + this.scaleLineLength + 4 : -this.outerRadius - this.scaleLineLength - 4,
                        y: 0,
                        text: Math.round(scaleValues.shift() * 10) / 10 + unitPostfix,
                        textAlign: isRightSide ? 'left' : 'right'
                    },
                    position: this.center.slice(),
                    rotation: isRightSide ? [thelta / 180 * Math.PI, 0, 0] : [(thelta + 180) / 180 * Math.PI, 0, 0]
                };
...
/*
            ...
            this.getColor = function (key) {
                ...
                for (var i = 0; i < this.chordSeries.length; i++) {
                    if (this.chordSeries[i].name === key) {
                        ...
                        this.chordSeries[i], 'itemStyle.normal.color');
                        break;
                    }
                }
                if (!colorMap[key]) {
                    ...
                    for (var i = 0; i < len; i++) {
                        if (this.groups[i].name === key) {
                            colorMap[key] = this.query(
                            this.groups[i], 'itemStyle.normal.color');
                            break;
                        }
                    }
                }
    ...
    reformOption: function (opt) {
        ...
        opt = _merge(
        opt || {}, this.ecTheme.chord);
        opt.itemStyle.normal.label.textStyle = _merge(
        opt.itemStyle.normal.label.textStyle || {}, this.ecTheme.textStyle);
    }
};
