...
var RibbonShape = require('../util/shape/Ribbon.js');
...
ecConfig.chord = {
    ...
    itemStyle: {
        normal: {
            ...
            chordStyle: { /** ribbonType = false 时有效 */
                width: 1,
                color: 'black',
                /** ribbonType = true 时有效 */
                borderWidth: 1,
...
var Graph = require('../data/Graph.js');
var ChordLayout = require('../layout/Chord.js');
...
Chord.prototype = {
        ...
        for (var i = 0, l = series.length; i < l; i++) {
            if (series[i].type === this.type) {
                if (series[i].insertToSerie) {
                    ...
                    if (
                    chordSeriesGroups[mainSerie.name] && this.selectedMap[series[i].name]) {
    ...
    _getEdgeQueryTarget: function (serie, edge, type) {
        type = type || 'normal';
        return [(edge.itemStyle && edge.itemStyle[type]), serie.itemStyle[type].chordStyle];
    },
    ...
    _buildSectors: function (serie, serieIdx, graph, mainSerie) {
        ...
        graph.eachNode(function (node) {
            ...
            sector.style.lineWidth = this.deepQuery([node.data, mainSerie], 'itemStyle.normal.borderWidth');
            sector.highlightStyle.lineWidth = this.deepQuery([node.data, mainSerie], 'itemStyle.emphasis.borderWidth');
            sector.style.strokeColor = this.deepQuery([node.data, mainSerie], 'itemStyle.normal.borderColor');
            sector.highlightStyle.strokeColor = this.deepQuery([node.data, mainSerie], 'itemStyle.emphasis.borderColor');
            ...
            ecData.pack(
            sector, serie, serieIdx, node.data, node.rawIndex, node.id,
            // special
            node.category);
    ...
    _buildNodeIcons: function (serie, serieIdx, graph, mainSerie) {
        ...
        graph.eachNode(function (node) {
            ...
            ecData.pack(
            iconShape, serie, serieIdx, node.data, node.rawIndex, node.id,
            // special
            node.category);
    ...
    _buildLabels: function (serie, serieIdx, graph, mainSerie) {
        ...
        graph.eachNode(function (node) {
            ...
            labelShape.style.textColor = this.deepQuery([node.data, mainSerie], 'itemStyle.normal.label.textStyle.color') || '#fff';
            labelShape.style.textFont = this.getFont(this.deepQuery([node.data, mainSerie], 'itemStyle.normal.label.textStyle'));
    ...
    _buildRibbons: function (series, serieIdx, graph, mainSerie) {
        ...
        graph.eachEdge(function (edge, idx) {
            ...
            ecData.pack(
            ribbon, serie, serieIdx, edge.data, edge.rawIndex == null ? idx : edge.rawIndex, edge.data.name || (node1.id + '-' + node2.id),
            // special
            node1.id,
            // special2
            node2.id);
    ...
    _buildEdgeCurves: function (series, serieIdx, graph, mainSerie, mainGraph) {
        ...
        graph.eachEdge(function (e, idx) {
            ...
            ecData.pack(
            curveShape, serie, serieIdx, e.data, e.rawIndex == null ? idx : e.rawIndex, e.data.name || (e.node1.id + '-' + e.node2.id),
            // special
            e.node1.id,
            // special2
            e.node2.id);
        ...
        graph.eachNode(function (node) {
            ...
            while (true) {
                ...
                var textShape = new TextShape({
                    ...
                    rotation: isRightSide ? [-theta / 180 * Math.PI, 0, 0] : [-(theta + 180) / 180 * Math.PI, 0, 0]
                });
    ...
    reformOption: function (opt) {
        ...
        opt = _merge(
        _merge(
        opt || {}, this.ecTheme.chord), ecConfig.chord);
