...
Map.prototype = {
        ...
        for (var mt in valueData) {
            if (valueCalculation[mt] && valueCalculation[mt] == 'average') {
                for (var k in valueData[mt]) {
                    valueData[mt][k].value = (valueData[mt][k].value / valueData[mt][k].seriesIndex.length).toFixed(
                    mapValuePrecision[mt]) - 0;
                }
            }
    ...
    _getProjectionData: function (mapType, mapData, mapSeries) {
        ...
        if (!this._mapDataMap[mapType].hasRoam) {
            ...
            bbox, mapSeries, this._mapDataMap[mapType].rate);
        }
        else {
            //经过用户漫游不再响应resize
...
/* for test
        ...
        if (this._specialArea[mapType]) {
            for (var area in this._specialArea[mapType]) {
                ...
                mapType, mapData, area, this._specialArea[mapType][area], position));
            }
        ...
        if (mapType == 'china') {
            ...
            mapType, _geoCoord['南海诸岛'] || _mapParams['南海诸岛'].textCoord);
            // scale.x : width  = 10.51 : 64
            var scale = transform.scale.x / 10.5;
            var textPosition = [
            32 * scale + leftTop[0], 83 * scale + leftTop[1]];
            if (_textFixed['南海诸岛']) {
    ...
    _getSingleProvince: function (mapType, path, position) {
        ...
        if (_geoCoord[name]) {
            ...
            mapType, _geoCoord[name]);
        }
        else if (path.cp) {
            textPosition = [
            path.cp[0] + textFixed[0], path.cp[1] + textFixed[1]];
        }
        else {
...
/*
            ...
            this.deepQuery(queryTarget, 'itemStyle.normal.color'), data.seriesIndex, -1, data) || this.deepQuery(
            ...
            this.deepQuery(queryTarget, 'itemStyle.emphasis.color'), data.seriesIndex, -1, data) || this.deepQuery(
            queryTarget, 'itemStyle.emphasis.areaStyle.color') || style.color;
            highlightStyle.strokeColor = this.deepQuery(
            queryTarget, 'itemStyle.emphasis.borderColor') || style.strokeColor;
            highlightStyle.lineWidth = this.deepQuery(
            queryTarget, 'itemStyle.emphasis.borderWidth') || style.lineWidth;
            ...
            textShape = {
                ...
                style: {
                    ...
                    queryTarget, 'itemStyle.normal.label.textStyle.color') : 'rgba(0,0,0,0)',
            ...
            if (this.deepQuery(queryTarget, 'itemStyle.emphasis.label.show')) {
                ...
                queryTarget, 'itemStyle.emphasis.label.textStyle.color') || textShape.style.color;
                font = this.deepQuery(
                queryTarget, 'itemStyle.emphasis.label.textStyle') || font;
    ...
    getMarkCoord: function (seriesIndex, mpData) {
        ...
        this._seriesIndexToMapType[seriesIndex], mpData.geoCoord || _geoCoord[mpData.name]) : [0, 0];
    },
    ...
    getLabelText: function (name, value, queryTarget, status) {
        ...
        queryTarget, 'itemStyle.' + status + '.label.formatter');
        if (formatter) {
            if (typeof formatter == 'function') {
                return formatter.call(
                this.myChart, name, value);
            }
