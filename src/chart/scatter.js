    ...
    _buildPointList: function (pointList) {
        ...
        for (var seriesIndex in pointList) {
            ...
            if (serie.large && serie.data.length > serie.largeThreshold) {
                ...
                seriesPL, this.getItemStyleColor(
                this.query(
                serie, 'itemStyle.normal.color'), seriesIndex, -1) || this._sIndex2ColorMap[seriesIndex]));
                continue;
            }
...
/*
    ...
    _getSymbol: function (seriesIndex, dataIndex, name, x, y) {
        ...
        serie, seriesIndex, data, dataIndex, name, x, y, this._sIndex2ShapeMap[seriesIndex], rangColor, 'rgba(0,0,0,0)', 'vertical');
    ...
    getMarkCoord: function (seriesIndex, mpData) {
        ...
        if (mpData.type && (mpData.type == 'max' || mpData.type == 'min' || mpData.type == 'average')) {
            // 特殊值内置支持
            // 默认取纵值
            var valueIndex = typeof mpData.valueIndex != 'undefined' ? mpData.valueIndex : 1;
            pos = [
...
// 图表注册
require('../chart.js').define('scatter', Scatter);
