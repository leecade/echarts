        ...
        for (var i = 0, l = maxDataLength; i < l; i++) {
            ...
            for (var j = 0, k = locationMap.length; j < k; j++) {
                ...
                for (var m = 0, n = locationMap[j].length; m < n; m++) {
                    ...
                    curPLMap[seriesIndex].push([x, y, i, categoryAxis.getNameByIndex(i), x, baseYP]);
            ...
            for (var j = 0, k = locationMap.length; j < k; j++) {
                for (var m = 0, n = locationMap[j].length; m < n; m++) {
                    ...
                    if (this.deepQuery([data, serie, this.option], 'calculable')) {
                        symbolSize = this.deepQuery([data, serie], 'symbolSize');
                        lastYP += symbolSize * 2 + 5;
                        y = lastYP;
                        this.shapeList.push(this._getCalculableItem(
                        seriesIndex, i, categoryAxis.getNameByIndex(i), x, y, 'horizontal'));
                    }
                }
            }
        }
        ...
        for (var i = 0, l = maxDataLength; i < l; i++) {
            ...
            for (var j = 0, k = locationMap.length; j < k; j++) {
                ...
                for (var m = 0, n = locationMap[j].length; m < n; m++) {
                    ...
                    curPLMap[seriesIndex].push([x, y, i, categoryAxis.getNameByIndex(i), baseXP, y]);
            ...
            for (var j = 0, k = locationMap.length; j < k; j++) {
                for (var m = 0, n = locationMap[j].length; m < n; m++) {
                    ...
                    if (this.deepQuery([data, serie, this.option], 'calculable')) {
                        ...
                        x = lastXP;
                        this.shapeList.push(this._getCalculableItem(
                        seriesIndex, i, categoryAxis.getNameByIndex(i), x, y, 'vertical'));
                    }
                }
            }
        }
    ...
    _buildBorkenLine: function (seriesArray, pointList, categoryAxis, orient) {
        ...
        for (var sIdx = seriesArray.length - 1; sIdx >= 0; sIdx--) {
            ...
            if (serie.type == this.type && typeof seriesPL != 'undefined') {
                ...
                serie, 'itemStyle.normal.areaStyle') != 'undefined';
                ...
                for (var i = 0, l = seriesPL.length; i < l; i++) {
                    ...
                    if (!isLarge) { // 非大数据模式才显示拐点symbol
                        for (var j = 0, k = singlePL.length; j < k; j++) {
                            ...
                            if (this.deepQuery([data, serie], 'showAllSymbol') // 全显示
                            || (categoryAxis.isMainAxis(singlePL[j][2]) && this.deepQuery([data, serie], 'symbol') != 'none') // 主轴非空
                            || this.deepQuery([data, serie, this.option], 'calculable') // 可计算
                            ) {
                                this.shapeList.push(this._getSymbol(
                                seriesIndex, singlePL[j][2], // dataIndex
                                singlePL[j][3], // name
                                singlePL[j][0], // x
                                singlePL[j][1], // y
                                orient));
                    ...
                    brokenLineShape = new BrokenLineShape({
                        ...
                        style: {
                            ...
                            serie, 'itemStyle.normal.lineStyle.shadowColor'),
                            shadowBlur: this.query(
                            serie, 'itemStyle.normal.lineStyle.shadowBlur'),
                            shadowOffsetX: this.query(
                            serie, 'itemStyle.normal.lineStyle.shadowOffsetX'),
                            shadowOffsetY: this.query(
                            serie, 'itemStyle.normal.lineStyle.shadowOffsetY')
                        },
                        hoverable: false,
                        _main: true,
                    ...
                    ecData.pack(
                    brokenLineShape, series[seriesIndex], seriesIndex, 0, i, series[seriesIndex].name);
                    ...
                    if (isFill) {
                        halfSmoothPolygonShape = new HalfSmoothPolygonShape({
                            ...
                            style: {
                                ...
                                pointList: zrUtil.clone(singlePL).concat([
                                    [
                                    singlePL[singlePL.length - 1][4], singlePL[singlePL.length - 1][5]],
                                    [
                                    singlePL[0][4], singlePL[0][5]]
                                ]),
                                brushType: 'fill',
                                smooth: this._getSmooth(serie.smooth),
                        ...
                        ecData.pack(
                        halfSmoothPolygonShape, series[seriesIndex], seriesIndex, 0, i, series[seriesIndex].name);
...
/* 不科学啊，发现0.3通用了
    ...
    _getSymbol: function (seriesIndex, dataIndex, name, x, y, orient) {
        ...
        serie, seriesIndex, data, dataIndex, name, x, y, this._sIndex2ShapeMap[seriesIndex], this._sIndex2ColorMap[seriesIndex], '#fff', orient == 'vertical' ? 'horizontal' : 'vertical' // 翻转
        );
                ...
                if (this.shapeList[i]._main && this.shapeList[i].style.pointList.length > 1) {
                    ...
                    if (aniMap[seriesIndex][2]) {
                        ...
                        if (this.shapeList[i].type == 'polygon') {
                            ...
                            isHorizontal ? (this.shapeList[i].style.pointList[len - 3][0] = pointList[len - 4][0]) : (this.shapeList[i].style.pointList[len - 3][1] = pointList[len - 4][1]);
