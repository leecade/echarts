...
Bar.prototype = {
        ...
        for (var i = 0, l = maxDataLength; i < l; i++) {
            ...
            for (var j = 0, k = locationMap.length; j < k; j++) {
                ...
                for (var m = 0, n = locationMap[j].length; m < n; m++) {
                    ...
                    barMaxWidthMap[seriesIndex] || Number.MAX_VALUE, barWidthMap[seriesIndex] || barWidth);
                    if (value === '-') {
                        // 空数据在做完后补充拖拽提示框
                        continue;
                    }
                    if (value > 0) {
                        // 正向堆积
                        barHeight = m > 0 ? valueAxis.getCoordSize(value) : (
                        isHorizontal ? (baseP - valueAxis.getCoord(value)) : (valueAxis.getCoord(value) - baseP));
                        // 非堆积数据最小高度有效
                        ...
                        // 负向堆积
                        barHeight = m > 0 ? valueAxis.getCoordSize(value) : (
                        isHorizontal ? (valueAxis.getCoord(value) - baseN) : (baseN - valueAxis.getCoord(value)));
                        // 非堆积数据最小高度有效
                        if (n === 1 && barMinHeightMap[seriesIndex] > barHeight) {
                            barHeight = barMinHeightMap[seriesIndex];
                        }
                    ...
                    if (i % interval === 0) {
                        ...
                        seriesIndex, i, categoryAxis.getNameByIndex(i), x, y - (isHorizontal ? 0 : curBarWidth), isHorizontal ? curBarWidth : barHeight, isHorizontal ? barHeight : curBarWidth, isHorizontal ? 'vertical' : 'horizontal');
                ...
                for (var m = 0, n = locationMap[j].length; m < n; m++) {
                    ...
                    if (this.deepQuery([data, serie, this.option], 'calculable')) {
                        ...
                        barShape = this._getBarItem(
                        seriesIndex, i, categoryAxis.getNameByIndex(i), x, y - (isHorizontal ? 0 : curBarWidth), isHorizontal ? curBarWidth : islandR, isHorizontal ? islandR : curBarWidth, isHorizontal ? 'vertical' : 'horizontal');
            ...
            for (var m = 0, n = locationMap[j].length; m < n; m++) {
                ...
                for (var i = 0, l = serie.data.length; i < l; i++) {
                    ...
                    if (barHeight != null) {
                        ...
                        barShape = this._getBarItem(
                        seriesIndex, i, value[0], x, y - barHeight / 2, barWidth, barHeight, orient);
                    }
                    else {
                        // 柱形
                        orient = 'vertical';
                        ...
                        barShape = this._getBarItem(
                        seriesIndex, i, value[0], x - barWidth / 2, y, barWidth, barHeight, orient);
                    }
                    this.shapeList.push(new RectangleShape(barShape));
    ...
    _mapSize: function (categoryAxis, locationMap, ignoreUserDefined) {
        ...
        if (locationMap.length != sBarWidthCounter) {
            ...
            if (!ignoreUserDefined) {
                ...
                if (typeof barGap === 'string' && barGap.match(/%$/)) {
                    ...
                    barWidth = +((gap - sBarWidthTotal - barGap * (locationMap.length - 1)) / (
                    locationMap.length - sBarWidthCounter)).toFixed(2);
                }
        ...
        locationMap, barWidthMap, barMaxWidthMap, barMinHeightMap, gap, // 总宽度
    ...
    _getBarItem: function (seriesIndex, dataIndex, name, x, y, width, height, orient) {
        ...
        barShape = {
            ...
            style: {
                ...
                this.deepQuery(queryTarget, 'itemStyle.normal.color') || defaultColor, seriesIndex, dataIndex, data),
            ...
            highlightStyle: {
                color: this.getItemStyleColor(
                this.deepQuery(queryTarget, 'itemStyle.emphasis.color'), seriesIndex, dataIndex, data),
