...
/* 调试输出
        ...
        for (var i = 0, l = maxDataLength; i < l; i++) {
            ...
            for (var j = 0, k = locationMap.length; j < k; j++) {
                ...
                for (var m = 0, n = locationMap[j].length; m < n; m++) {
                    ...
                    data = serie.data[i];
                    value = typeof data != 'undefined' ? (typeof data.value != 'undefined' ? data.value : data) : '-';
                    ...
                    if (value > 0) {
                        // 正向堆积
                        //barHeight = baseYP - y;
                        barHeight = m > 0 ? valueAxis.getCoordSize(value) : (baseYP - valueAxis.getCoord(value));
                        // 非堆积数据最小高度有效
                        ...
                        // 负向堆积
                        //barHeight = y - baseYN;
                        barHeight = m > 0 ? valueAxis.getCoordSize(value) : (valueAxis.getCoord(value) - baseYN);
                        // 非堆积数据最小高度有效
                        if (n == 1 && barMinHeightMap[seriesIndex] > barHeight) {
                            barHeight = barMinHeightMap[seriesIndex];
                        }
                    ...
                    if (i % interval === 0) {
                        ...
                        seriesIndex, i, categoryAxis.getNameByIndex(i), x, y, barWidthMap[seriesIndex] || barWidth, barHeight, 'vertical');
                ...
                for (var m = 0, n = locationMap[j].length; m < n; m++) {
                    ...
                    if (this.deepQuery([data, serie, this.option], 'calculable')) {
                        ...
                        barShape = this._getBarItem(
                        seriesIndex, i, categoryAxis.getNameByIndex(i), x + 0.5, y + 0.5, (barWidthMap[seriesIndex] || barWidth) - 1, this.ecTheme.island.r - 1, 'vertical');
                        ...
                        serie.calculableHolderColor || this.ecTheme.calculableHolderColor;
        ...
        for (var j = 0, k = locationMap.length; j < k; j++) {
            for (var m = 0, n = locationMap[j].length; m < n; m++) {
                ...
                if (xMarkMap[seriesIndex].counter > 0) {
                    xMarkMap[seriesIndex].average = (xMarkMap[seriesIndex].sum / xMarkMap[seriesIndex].counter).toFixed(2) - 0;
                }

                y = this.component.yAxis.getAxis(series[seriesIndex].yAxisIndex || 0).getCoord(xMarkMap[seriesIndex].average);
            ...
            for (var j = 0, k = locationMap.length; j < k; j++) {
                ...
                for (var m = 0, n = locationMap[j].length; m < n; m++) {
                    ...
                    if (value > 0) {
                        // 正向堆积
                        //barHeight = x - baseXP;
                        barHeight = m > 0 ? valueAxis.getCoordSize(value) : (valueAxis.getCoord(value) - baseXP);
                        // 非堆积数据最小高度有效
                        if (n == 1 && barMinHeightMap[seriesIndex] > barHeight) {
                            barHeight = barMinHeightMap[seriesIndex];
                        }
                        ...
                        // 负向堆积
                        //barHeight = baseXN - x;
                        barHeight = m > 0 ? valueAxis.getCoordSize(value) : (baseXN - valueAxis.getCoord(value));
                        // 非堆积数据最小高度有效
                        if (n == 1 && barMinHeightMap[seriesIndex] > barHeight) {
                            barHeight = barMinHeightMap[seriesIndex];
                        }
                    ...
                    if (i % interval === 0) {
                        barShape = this._getBarItem(
                        seriesIndex, i, categoryAxis.getNameByIndex(i), x, y - (barWidthMap[seriesIndex] || barWidth), barHeight, barWidthMap[seriesIndex] || barWidth, 'horizontal');
                        this.shapeList.push(new RectangleShape(barShape));
                    }
                }
                ...
                for (var m = 0, n = locationMap[j].length; m < n; m++) {
                    ...
                    if (this.deepQuery([data, serie, this.option], 'calculable')) {
                        ...
                        barShape = this._getBarItem(
                        seriesIndex, i, categoryAxis.getNameByIndex(i), x + 0.5, y + 0.5 - (barWidthMap[seriesIndex] || barWidth), this.ecTheme.island.r - 1, (barWidthMap[seriesIndex] || barWidth) - 1, 'horizontal');
                        barShape.hoverable = false;
                        barShape.draggable = false;
                        barShape.style.lineWidth = 1;
                        barShape.style.brushType = 'stroke';
    ...
    _mapSize: function (categoryAxis, locationMap, ignoreUserDefined) {
        ...
        for (var j = 0, k = locationMap.length; j < k; j++) {
            ...
            for (var m = 0, n = locationMap[j].length; m < n; m++) {
                ...
                queryTarget, 'barMinHeight');
                barGap = typeof barGap != 'undefined' ? barGap : this.query(
                queryTarget, 'barGap');
                barCategoryGap = typeof barCategoryGap != 'undefined' ? barCategoryGap : this.query(
                queryTarget, 'barCategoryGap');
            }
        }
        ...
        if (locationMap.length != sBarWidthCounter) {
            ...
            if (!ignoreUserDefined) {
                ...
                // 百分比
                ? Math.floor(
                categoryAxis.getGap() * (100 - parseFloat(barCategoryGap)) / 100)
                // 数值
            ...
            gap = sBarWidthCounter > 1 ? (typeof barCategoryGap == 'string' && barCategoryGap.match(/%$/))
            // 百分比
            ? Math.floor(
            categoryAxis.getGap() * (100 - parseFloat(barCategoryGap)) / 100)
            // 数值
    ...
    _getBarItem: function (seriesIndex, dataIndex, name, x, y, width, height, orient) {
        ...
        queryTarget, 'itemStyle.normal.color') || defaultColor;
        ...
        ecData.pack(
        barShape, series[seriesIndex], seriesIndex, series[seriesIndex].data[dataIndex], dataIndex, name);
    ...
    getMarkCoord: function (seriesIndex, mpData) {
        ...
        if (mpData.type && (mpData.type == 'max' || mpData.type == 'min' || mpData.type == 'average')) {
            // 特殊值内置支持
            pos = [
            xMarkMap[mpData.type + 'X'], xMarkMap[mpData.type + 'Y'], xMarkMap[mpData.type + 'Line'], xMarkMap[mpData.type]];
        }
            ...
            x = typeof x != 'undefined' ? x : typeof mpData.xAxis != 'string' && xAxis.getCoordByIndex ? xAxis.getCoordByIndex(mpData.xAxis || 0) : xAxis.getCoord(mpData.xAxis || 0);
            ...
            y = typeof y != 'undefined' ? y : typeof mpData.yAxis != 'string' && yAxis.getCoordByIndex ? yAxis.getCoordByIndex(mpData.yAxis || 0) : yAxis.getCoord(mpData.yAxis || 0);
            ...
            if (aniMap[seriesIndex] && !aniMap[seriesIndex][3]) {
                ...
                if (this.shapeList[i].type == 'rectangle') {
                    ...
                    if (this.shapeList[i]._orient == 'horizontal') {
                        ...
                        serie.xAxisIndex || 0).getGap();
                        x = aniMap[seriesIndex][2] ? dx : -dx;
                        y = 0;
                    }
                    this.shapeList[i].position = [0, 0];
                    this.zr.animate(this.shapeList[i].id, '').when(
                    500, {
                        position: [x, y]
                    }).start();
                }
