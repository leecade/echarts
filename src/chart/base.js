...
/* 调试输出
    ...
    _buildMarkLine: function (seriesIndex) {
        ...
        for (var i = 0, l = markLine.data.length; i < l; i++) {
            ...
            if (mlData.type && (mlData.type === 'max' || mlData.type === 'min' || mlData.type === 'average')) {
                ...
                markLine.precision != null ? markLine.precision : this.deepQuery([this.ecTheme, ecConfig], 'markLine.precision'));
                pos = pos[2];
                mlData = [{},
                {}];
            }
    ...
    _markPoint: function (seriesIndex, mpOption) {
        ...
        zrUtil.merge(
        zrUtil.merge(
        mpOption, zrUtil.clone(this.ecTheme.markPoint || {})), zrUtil.clone(ecConfig.markPoint));
    ...
    _markLine: function (seriesIndex, mlOption) {
        ...
        zrUtil.merge(
        zrUtil.merge(
        mlOption, zrUtil.clone(this.ecTheme.markLine || {})), zrUtil.clone(ecConfig.markLine));
        ...
        for (var i = 0, l = data.length; i < l; i++) {
            ...
            mlOption, // markLine
            seriesIndex, mlData, // 数据
            i, this.parsePercent(mlData[0].x, zrWidth), // 坐标
            ...
            if (serie.type === ecConfig.CHART_TYPE_MAP) {
                itemShape._geo = [
                this.getMarkGeo(mlData[0]), this.getMarkGeo(mlData[1])];
            }

            // 重新pack一下数据
            ecData.pack(
            itemShape, serie, seriesIndex, mlData[0], i, mlData[0].name
            // 不要帮我代码规范
            + (mlData[1].name !== '' ? (' > ' + mlData[1].name) : ''), value);
            pList.push(itemShape);
        }
        //console.log(pList);
        return pList;
