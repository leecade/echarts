...
function isInside(shape, area, x, y) {
    ...
    if (zoneType != 'bezier-curve' && shape.buildPath && _ctx.isPointInPath) {
...
function _isPainted(context, x, y, unit) {
    ...
    if (typeof unit != 'undefined') {
        ...
        x - unit, y - unit, unit + unit, unit + unit).data;
    }
    else {
...
function _isInsideRing(area, x, y) {
    return _isInsideCircle(area, x, y, area.r) && !_isInsideCircle({
        x: area.x,
        y: area.y
    }, x, y, area.r0 || 0);
}
