...
function _mathMethod(shape, area, x, y) {
    ...
    case 'bezier-curve':
        if (typeof(area.cpX2) === 'undefined') {
            ...
            area.xStart, area.yStart, area.cpX1, area.cpY1, area.xEnd, area.yEnd, area.lineWidth, x, y);
        }
        return isInsideCubicStroke(
        area.xStart, area.yStart, area.cpX1, area.cpY1, area.cpX2, area.cpY2, area.xEnd, area.yEnd, area.lineWidth, x, y);
        // 线
    case 'line':
        return isInsideLine(
        area.xStart, area.yStart, area.xEnd, area.yEnd, area.lineWidth, x, y);
        // 折线
    case 'polyline':
    ...
    case 'sector':
        ...
        area.x, area.y, area.r0, area.r, startAngle, endAngle, !area.clockWise, x, y);
        // 多边形
    case 'path':
        return area.pathArray && isInsidePath(
        area.pathArray, Math.max(area.lineWidth, 5), area.brushType, x, y);
    case 'polygon':
    case 'star':
    case 'isogon':
...
x0, y0, x1, y1, x2, y2, x3, y3, lineWidth, x, y) {
    if (lineWidth === 0) {
        return false;
    }
    var _l = Math.max(lineWidth, 5);
    ...
    x0, y0, x1, y1, x2, y2, x3, y3, x, y, null);
    return d <= _l / 2;
}
...
x0, y0, x1, y1, x2, y2, lineWidth, x, y) {
    if (lineWidth === 0) {
        return false;
    }
    var _l = Math.max(lineWidth, 5);
...
cx, cy, r, startAngle, endAngle, anticlockwise, lineWidth, x, y) {
    if (lineWidth === 0) {
        return false;
    }
    var _l = Math.max(lineWidth, 5);
...
cx, cy, r0, r, startAngle, endAngle, anticlockwise, x, y) {
    ...
    cx, cy, (r0 + r) / 2, startAngle, endAngle, anticlockwise, r - r0, x, y);
}
    ...
    for (var i = 0; i < pathArray.length; i++) {
        ...
        case 'C':
            if (hasStroke) {
                ...
                xi, yi, p[0], p[1], p[2], p[3], p[4], p[5], lineWidth, x, y)) {
                    return true;
                }
            }
            if (hasFill) {
        ...
        case 'Q':
            if (hasStroke) {
                ...
                xi, yi, p[0], p[1], p[2], p[3], lineWidth, x, y)) {
                    return true;
                }
            }
            if (hasFill) {
        ...
        case 'A':
            ...
            if (hasStroke) {
                ...
                cx, cy, ry, theta, theta + dTheta, 1 - p[7], lineWidth, _x, y)) {
                    return true;
                }
            }
            if (hasFill) {
                w += windingArc(
                cx, cy, ry, theta, theta + dTheta, 1 - p[7], _x, y);
            }
...
function getTextWidth(text, textFont) {
    ...
    for (var i = 0, l = text.length; i < l; i++) {
        width = Math.max(
        _ctx.measureText(text[i]).width, width);
    }
    _ctx.restore();
