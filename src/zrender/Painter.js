            ...
            if (!shape.onbrush || (shape.onbrush && !shape.onbrush(ctx, false))) {
                if (config.catchBrushException) {
                    try {
                        ...
                        log(
                        error, 'brush error of ' + shape.type, shape);
                    }
                } else {
                    shape.brush(ctx, false, this.updatePainter);
                }
...
Painter.prototype.getLayer = function (zlevel, prevLayer) {
    ...
    if (!currentLayer) {
        ...
        if (prevDom.nextSibling) {
            ...
            currentLayer.dom, prevDom.nextSibling);
        } else {
...
Painter.prototype.toDataURL = function (type, backgroundColor, args) {
    ...
    ctx.rect(
    0, 0, this._width * devicePixelRatio, this._height * devicePixelRatio);
    ctx.fill();
...
Layer.prototype.clear = function () {
    ...
    if (config) {
        var haveClearColor =
        typeof(config.clearColor) !== 'undefined' && !vmlCanvasManager;
        ...
        if (haveMotionBLur) {
            ...
            dom, 0, 0, width / devicePixelRatio, height / devicePixelRatio);
        }
        ...
        if (haveMotionBLur) {
            ...
            ctx.drawImage(
            domBack, 0, 0, width / devicePixelRatio, height / devicePixelRatio);
            ctx.restore();
        }
    }
    else {
